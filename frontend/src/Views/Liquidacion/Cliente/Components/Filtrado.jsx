...
function Filtrado({ onSearch }) {
  ...
  useEffect(() => {
    fetch(
      "https://sysdemo03.byma-ve.com/BackendApiRest/Administracion/Cliente/obtener_clientes.php"
    )
  ...
  useEffect(() => {
    if (idCliente) {
      fetch(
        `https://sysdemo03.byma-ve.com/BackendApiRest/Administracion/Area/obtenerAreasTarifario.php?id_cliente=${idCliente}`
      )
        .then((response) => response.json())
  ...
  async function descargarExcel() {
    ...
    try {
      ...
      const apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Liquidacion/LiquidacionCliente/exportarLiquidiacionCliente.php?id_cliente=${idCliente}&id_area=${idArea}&fechaDesde=${fechaDesde}&fechaHasta=${fechaHasta}`;
      ...
      if (Array.isArray(apiData)) {
        for (let i = 0; i < apiData.length; i++) {
          ...
          worksheet.getCell(`J${rowNumber}`).value = rowData.estado_ultimo_intento;
        ...
        <div className="flex justify-between">
          ...
          <div className="cont-fecha flex justify-between ">
            ...
            <div className=" w-full  ">
              <SearchArea areas={areas} idCliente={idCliente} handleSelectArea={handleSelectArea} />
            </div>
