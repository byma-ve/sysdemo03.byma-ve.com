...
import AgregarCotizacion from "../Modals/ModalAgregar";
import ModalEditar from "../Modals/ModalEditar";
import { IconoAgregar, IconoCerrar, IconoPencil } from "../../../../Iconos/Iconos-NavBar";
function ListaDestinos({ clienteElegido, areaElegida, onFetchData, opcionesSelect, cargarListaEnvios }) {
  ...
  const eliminarDestino = async (id) => {
    try {
      const response = await fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Comercial/Cotizacion/eliminarDestino.php?id=${id}`, {
        method: 'DELETE',
      });
  ...
  const obtenerValoresDestino = async (destinoSeleccionado) => {
    try {
      const response = await fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Comercial/Cotizacion/obtenerDestino.php?id_destino_cotizacion=${destinoSeleccionado}`);
