...
function Table({
  ...
  useEffect(() => {
    if (registroSeleccionado != null) {
      fetch(
        `https://sysdemo03.byma-ve.com/BackendApiRest/Comercial/PuntoVenta/imprimirPdf.php?id_cotizacion=${registroSeleccionado}`
      )
              ...
              {cotizacionesFiltrados &&
                cotizacionesActuales.map((cotizacion, index) => (
                  ...
                  >
                    <th
                      scope="row"
                      className=" p-[0.75rem_1.5rem] font-medium whitespace-nowrap text-gray-700 w-[4rem] max-w-[64px]"
                    >
                      <button
                        onClick={() => {
