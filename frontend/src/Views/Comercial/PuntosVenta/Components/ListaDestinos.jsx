...
function ListaDestinos({ clienteElegido, areaElegida, onFetchData, opcionesSelect, cargarListaEnvios }) {
  ...
  const eliminarDestino = async (id) => {
    try {
      const response = await fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Comercial/PuntoVenta/eliminarDestino.php?id=${id}`, {
        method: 'DELETE',
      });
  ...
  const obtenerValoresDestino = async (destinoSeleccionado) => {
    try {
      const response = await fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Comercial/PuntoVenta/obtenerDestino.php?id_destino_cotizacion=${destinoSeleccionado}`);
