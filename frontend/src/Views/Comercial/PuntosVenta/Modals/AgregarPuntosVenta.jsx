...
function Agregar({
  ...
  useEffect(() => {
    ...
    if (distritoSeleccionada || tipoTarifario) {
      fetch(
        `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Cliente/obtenerTarifarioUbigeo.php?id_cliente=${id_cliente}&id_area=${id_area}&ubigeo=${distritoSeleccionada}&tarifario=${tipoTarifario}`
      )
        .then((response) => response.json())
        .then((data) => {
    ...
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://sysdemo03.byma-ve.com/BackendApiRest/Comercial/PuntoVenta/totalTarifa.php`,
          {
            method: "POST",
            headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(
          `https://sysdemo03.byma-ve.com/BackendApiRest/Comercial/PuntoVenta/cargosAdicionales.php`,
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const handleSubmit = async (e) => {
    ...
    `,
      ...
      try {
        const response = await fetch(
          "https://sysdemo03.byma-ve.com/BackendApiRest/Comercial/PuntoVenta/guardarDestino.php",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  return (
    <>
      <div
        className={`side-panel-container ${modalCotizacion ? "visible" : "invisible"
          } fixed pointer-events-auto left-0 top-0 right-0 h-full bg-[rgba(0,0,0,0.4)] z-10 flex 
        justify-center items-center `}
      >
        <div
          className={`side-panel-cont-container ${modalCotizacion ? "translate-y-0" : "translate-y-[600%]"
              ...
              <div className="side-panel bg-white  rounded-md h-full w-auto m-0   ">
                <div className="side-cont-titulo mb-4 text-[25px] px-5 py-2 rounded-t-md bg-blue-400 w-full font-semibold text-white">
                  <div className="side-titulo">
                    <h1 className="side-txt">Agregar Destino</h1>
                  </div>
                </div>
