  ...
  async function descargarTabla() {
    try {
      ...
      if (tarifaSeleccionada === "Courier") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Agente/exportarCourrier.php?id_agente=${encodeURIComponent(
          clienteSeleccionado?.id
        )}`;
      }
      if (tarifaSeleccionada === "Aereo") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Agente/exportarAereo.php?id_agente=${encodeURIComponent(
          clienteSeleccionado?.id
        )}`;
      }
  ...
  return (
    <>
      <div className="ml-1  px-4 pb-4 w-full">
        <div className="  h-full w-full flex justify-between bg-var(--gris) ">
          <input
      ...
      <ImportarPlantilla
        ...
        id_agente={clienteSeleccionado?.id || ""}
