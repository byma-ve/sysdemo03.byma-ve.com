...
export const ImportarPlantilla = ({ modalMasivo, setModalMasivo, id_cliente, id_area, tarifario, actualizarTabla }) => {
    ...
    reader.onload = async (e) => {
      ...
      try {
        ...
        if (tarifario === 'Courier' || tarifario === 'Aereo') {
          ...
          // VALIDACIÓN VALORIZADO
        } else if (tarifario === 'Valorizado') {
          const D1 = worksheet.getRow(1).getCell(4).value;
          if (String(D1).toUpperCase() !== "PRODUCTO") {
            throw new Error('La columna D1 no contiene PRODUCTO');
          }

          const E1 = worksheet.getRow(1).getCell(5).value;
          if (String(E1).toUpperCase() !== "S/ PRODUCTO") {
            throw new Error('La columna E1 no contiene S/ PRODUCTO');
          }
          ...
          const D1 = worksheet.getRow(1).getCell(4).value;
          if (String(D1).toUpperCase() !== "KG MAXIMO") {
            throw new Error('La columna D1 no contiene KG MAXIMO');
          }

          const E1 = worksheet.getRow(1).getCell(5).value;
          if (String(E1).toUpperCase() !== "KG BASE") {
            throw new Error('La columna E1 no contiene KG BASE');
          }

          const F1 = worksheet.getRow(1).getCell(6).value;
          if (String(F1).toUpperCase() !== "PAQUETE") {
            throw new Error('La columna F1 no contiene PAQUETE');
          }

          const G1 = worksheet.getRow(1).getCell(7).value;
          if (String(G1).toUpperCase() !== "T.MIN ENTREGA") {
            throw new Error('La columna G1 no contiene T.MIN ENTREGA');
          }

          const H1 = worksheet.getRow(1).getCell(8).value;
          if (String(H1).toUpperCase() !== "T.MAX ENTREGA") {
            throw new Error('La columna H1 no contiene T.MAX ENTREGA');
          }
        }
  ...
  const enviarDatos = async () => {
    ...
    try {
      ...
      if (tarifario === "Courier") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Cliente/guardarCourrier.php`;
      }
      if (tarifario === "Aereo") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Cliente/guardarAereo.php`;
      }
      if (tarifario === "Valorizado") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Cliente/guardarValorizado.php`;
      }
      if (tarifario === "Carga") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Cliente/guardarCarga.php`;
      }
