...
function ElegirAreaTarif({ seleccionarArea, id_cliente }) {
  ...
  useEffect(() => {
    if (id_cliente) {
      fetch(
        `https://sysdemo03.byma-ve.com/BackendApiRest/Administracion/Area/obtener_area.php?id_cliente=${encodeURIComponent(
          id_cliente
        )}`
      )
  ...
  return (
    <>
      <div className="z-10 absolute  overflow-y-auto   top-[191px] ml-2 text-left mt-1  rounded-xl shadow w-[13.8rem]  ">
        ...
        <ul className="p-3 overflow-y-auto text-sm rounded-xl -mt-1 max-h-[120px] text-gray-600 bg-slate-100 ScrollTableVertical">
