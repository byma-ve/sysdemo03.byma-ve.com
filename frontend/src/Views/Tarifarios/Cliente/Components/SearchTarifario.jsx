  ...
  async function descargarTabla() {
    try {
      ...
      if (tarifaSeleccionada === "Courier") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Cliente/exportarCourrier.php?id_cliente=${encodeURIComponent(
          clienteSeleccionado?.id
        )}&id_area=${encodeURIComponent(areaSeleccionada?.id)}`;
      }
      if (tarifaSeleccionada === "Aereo") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Cliente/exportarAereo.php?id_cliente=${encodeURIComponent(
          clienteSeleccionado?.id
        )}&id_area=${encodeURIComponent(areaSeleccionada?.id)}`;
      }
      if (tarifaSeleccionada === "Valorizado") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Cliente/exportarValorizado.php?id_cliente=${encodeURIComponent(
          clienteSeleccionado?.id
        )}&id_area=${encodeURIComponent(areaSeleccionada?.id)}`;
      }
      if (tarifaSeleccionada === "Carga") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Cliente/exportarCarga.php?id_cliente=${encodeURIComponent(
          clienteSeleccionado?.id
        )}&id_area=${encodeURIComponent(areaSeleccionada?.id)}`;
      }
  ...
  async function descargarUbigeo() {
    try {
      const response = await fetch(
        `https://sysdemo03.byma-ve.com/BackendApiRest/Ubigeo/exportarUbigeo.php`
      );
      const datos = await response.json();
