...
const SearchTarifario = ({ onOptionChange, onSearch, onOptionCliente, actualizarTabla }) => {
  ...
  async function descargarTabla() {
    try {
      ...
      if (tarifaSeleccionada === "Courier") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Transportista/exportarCourrier.php?id_transportista=${encodeURIComponent(clienteSeleccionado?.id)}`;
      }
      if (tarifaSeleccionada === "Carga") {
        apiUrl = `https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Transportista/exportarCarga.php?id_transportista=${encodeURIComponent(clienteSeleccionado?.id)}`;
      }
      ...
      if (datos.length > 0) {
        ...
        encabezados = ['Departamento', 'Provincia', 'Distrito', 'KG', 'KG Adicional', 'T.Min Entrega', 'T.Max Entrega'];
      } else if (tarifaSeleccionada === "Carga") {
        encabezados = ['Departamento', 'Provincia', 'Distrito', 'KG Maximo', 'KG Base', 'Paquete', 'T.Min Entrega', 'T.Max Entrega'];
      }
  ...
  async function descargarUbigeo() {
    try {
      const response = await fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Ubigeo/exportarUbigeo.php`);
      const datos = await response.json();
        ...
        <input
          ...
          <div className="flex ">
            ...
            <div className="flex">
              {mostrarExportar && (
                <div className="absolute bg-sky-50 rounded-lg shadow whitespace-nowrap right-[4.2%] top-[15rem] text-left z-10 ">
                  <ul className="p-3 overflow-y-auto text-sm text-gray-200">
                    <div onClick={descargarTabla} className="flex items-center  rounded hover:bg-gray-200">
                    ...
                    <div className="flex items-center  rounded hover:bg-gray-200">
                      <label onClick={descargarUbigeo} className="w-full py-1 px-3  text-sm font-medium text-gray-900 rounded cursor-pointer">
                        Exportar Ubigeo
                      </label>
                    </div>
                    <div
              ...
              border-none rounded-xl    ${!tarifarioButtonDisabled ? "" : "cursor-not-allowed"
                  }`}
