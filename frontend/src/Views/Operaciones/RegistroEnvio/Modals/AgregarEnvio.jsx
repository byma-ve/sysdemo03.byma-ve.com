...
function AgregarEnvio({ modalEnvio, setModalEnvio, id_cliente, id_area, cargarListaEnvios }) {
  ...
  useEffect(() => {
    ...
    fetch('https://sysdemo03.byma-ve.com/BackendApiRest/Ubigeo/select_ubigeo.php?action=departamentos')
  ...
  useEffect(() => {
    if (departamentoSeleccionado) {
      ...
      fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Ubigeo/select_ubigeo.php?action=provincias&id=${departamentoSeleccionado}`)
        .then(response => response.json())
  ...
  useEffect(() => {
    if (provinciaSeleccionada) {
      ...
      fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Ubigeo/select_ubigeo.php?action=distritos&id=${provinciaSeleccionada}`)
        .then(response => response.json())
  ...
  useEffect(() => {
    ...
    if (distritoSeleccionada || tipoTarifario) {
      fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Tarifarios/Cliente/obtenerTarifarioUbigeo.php?id_cliente=${id_cliente}&id_area=${id_area}&ubigeo=${distritoSeleccionada}&tarifario=${tipoTarifario}`)
        .then(response => response.json())
        .then(data => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Operaciones/RegistroEnvio/totalTarifa.php`, {
          method: 'POST',
          headers: {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Operaciones/RegistroEnvio/cargosAdicionales.php`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
          },
  ...
  const handleSubmit = async (e) => {
    ...
    `,
      ...
      try {
        const response = await fetch('https://sysdemo03.byma-ve.com/BackendApiRest/Operaciones/RegistroEnvio/guardarDestino.php', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
  ...
  return (
    <>
      <div
        className={`side-panel-container ${modalEnvio ? "visible" : "invisible"
          } fixed pointer-events-auto left-0 top-0 right-0 h-full bg-[rgba(0,0,0,0.4)] z-10 flex justify-center items-center`}
      >
        <div
          className={`side-panel-cont-container ${modalEnvio ? "translate-y-0" : "translate-y-[600%]"
              ...
              <div className="side-panel bg-white  rounded-md h-full w-auto m-0  ">
                ...
                <div className="section-crm pb-6 px-6">
                  <div className=" card-container  text-black">
                    <form className="mb-1 text-black" key={resetForm} onSubmit={handleSubmit}>
                      ...
                      <div className="grid grid-cols-4 pl-5 gap-0">
                        ...
                        <div className="ml-[-25px] mr-[20px]">
                          ...
                          >
                            ...
                            {provincias.map((provincia) => (
                              <option className="bg-gray-200" key={provincia.id} value={provincia.id}>
                        ...
                        <div className="ml-[-25px] mr-[20px]">
                          ...
                          >
                            ...
                            {distritos.map((distrito) => (
                              <option className="bg-gray-200" key={distrito.id} value={distrito.ubigeo}>
                        ...
                        <div className="ml-[-25px] mr-[20px]">
                          <input
                            ...
                            onChange={handleChange}
                            readOnly
                            value={datosTarifario.length > 0 ? datosTarifario[0].tmin : 'No existe Tarifario'}
                            className="w-[100%] border  px-1  rounded-sm text-xs h-5 focus:outline-none focus:ring-0  focus:border-blue-500  focus:shadow-md"
                          />
                        </div>
                        <div className="">
                        ...
                        <div className="ml-[-25px] mr-[20px]">
                          <input
                            readOnly
                            value={datosTarifario.length > 0 ? datosTarifario[0].tmax : 'No existe Tarifario'}
                            type="text"
                            onChange={handleChange}
                      ...
                      <div className="flex items-center justify-end mt-8 h-5">
                        ...
                        <button
                          onClick={() => { ocultarModalAgregar(); resetFormState(); }}
                          type="button"
