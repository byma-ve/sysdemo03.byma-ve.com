...
import AgregarEnvio from "../Modals/AgregarEnvio";
...
function ListaDestinos({ clienteElegido, areaElegida, opcionesSelect, cargarListaEnvios }) {
  ...
  const eliminarDestino = async (id) => {
    try {
      const response = await fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Operaciones/RegistroEnvio/eliminarDestino.php?id=${id}`, {
        method: 'DELETE',
      });
  ...
  const obtenerValoresDestino = async (destinoSeleccionado) => {
    try {
      const response = await fetch(`https://sysdemo03.byma-ve.com/BackendApiRest/Operaciones/RegistroEnvio/obtenerDestino.php?id_destino_cotizacion=${destinoSeleccionado}`);
