...
function TrabPropios({ formValues, handleChange }) {
  ...
  const handleConductorChange = (conductor) => {
    ...
    if (conductor) {
      fetch(
        `https://sysdemo03.byma-ve.com/BackendApiRest/Administracion/Usuario/datosConductor.php?id=${conductor.id}`
      )
        .then((response) => response.json())
        .then((data) => {
  ...
  const handleAuxiliarChange = (auxiliar) => {
    ...
    if (auxiliar) {
      fetch(
        `https://sysdemo03.byma-ve.com/BackendApiRest/Administracion/Usuario/datosAuxiliar.php?id=${auxiliar.id}`
      )
        .then((response) => response.json())
        .then((data) => {
