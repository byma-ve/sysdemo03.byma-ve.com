...
export function ModalDatosColaboradores({ modalDatos, setModalDatos, datosOrdenServicio, cargarSelect, setDatosOrdenServicio, setOpcionRecojoSeleccionado, setDatosSelect }) {
  ...
  useEffect(() => {
    setFormValues({
      id_orden_servicio_estado_recojo: datosOrdenServicio?.id_orden_servicio_recojo || '',
      proceso_estado_recojo: datosOrdenServicio?.proceso_estado_recojo || 'Recojo',
      estado_mercancia_estado_recojo: datosOrdenServicio?.estado_mercancia_estado_recojo || 'Exitoso',
      comentario_estado_recojo: datosOrdenServicio?.comentario_estado_recojo || '',
  ...
  const handleSubmit = async (e) => {
    ...
    formDataWithImage.append("id_orden_servicio_estado_recojo", formValues.id_orden_servicio_estado_recojo);
    formDataWithImage.append("proceso_estado_recojo", formValues.proceso_estado_recojo);
    formDataWithImage.append("estado_mercancia_estado_recojo", formValues.estado_mercancia_estado_recojo);
    formDataWithImage.append("comentario_estado_recojo", formValues.comentario_estado_recojo);
    formDataWithImage.append("fecha_creado", formValues.fecha_creado);
    formDataWithImage.append("imagen_estado_recojo", imagenCliente);
    formDataWithImage.append("id_creador_estado_recojo", localStorage.getItem('id_usuario'));
    ...
    try {
      const response = await fetch('https://sysdemo03.byma-ve.com/BackendApiRest/Operaciones/EstadoRecojos/guardarRecojo.php', {
        method: 'POST',
  ...
  return (
    <>
      <div
        className={`side-panel-container ${modalDatos ? "visible" : "invisible"
          } fixed pointer-events-auto left-0 top-0 right-0 h-full bg-[rgba(0,0,0,0.4)] z-10 flex justify-center items-center`}
      >
        <div
          className={`side-panel-cont-container ${modalDatos ? "translate-y-0" : "translate-y-[600%]"
              ...
              <div className="side-panel bg-white  rounded-md h-full w-auto m-0   ">
                ...
                <div className="side-cont-titulo mb-4 text-[25px] px-5 py-2   rounded-t-md bg-blue-400 w-full font-semibold text-white">
                  <div className="side-titulo">
                    <h1 className="side-txt">Instancia - {formValues.id_orden_servicio_estado_recojo}</h1>
                  </div>
                </div>
                <div className="section-crm pb-6 px-6">
                  <div className=" card-container text-black">
                    <form className="mb-1 text-black" onSubmit={handleSubmit} encType="multipart/form-data">
                        ...
                        <div className="py-2">
                          <div>
                            ...
                            <textarea
                              ...
                              className="uppercase w-[305%] rounded-sm my-2 bg-gray-100 px-1 border border-gray-300 focus:outline-none   focus:border-blue-600"
                              required
                            />
                          </div>
                          <div>
                            ...
                            <input
                              className="my-2 w-[305%] block text-sm   border-gray-100 rounded-sm cursor-pointer bg-gray-100 dark:text-gray-400 focus:outline-none dark:bg-gray-100 dark:border-gray-400 dark:placeholder-gray-400 "
