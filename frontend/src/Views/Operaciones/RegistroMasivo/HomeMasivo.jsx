...
import TablaMasivo from "./Components/Table";
...
function HomeMasivo() {
  ...
  const cargarListaEnvios = async () => {
    try {
      const response = await fetch(
        `https://sysdemo03.byma-ve.com/BackendApiRest/Operaciones/RegistroMasivo/obtenerListaDestinos.php?id_cliente=${clienteElegido}&id_area=${areaElegida}`
      );
  ...
  const cargarTabla = async () => {
    try {
      const response = await fetch(
        "https://sysdemo03.byma-ve.com/BackendApiRest/Operaciones/RegistroMasivo/obtenerCotizaciones.php"
      );
      const data = await response.json();
  ...
  const eliminarCotizacion = async (id) => {
    try {
      const response = await fetch(
        `https://sysdemo03.byma-ve.com/BackendApiRest/Operaciones/RegistroMasivo/eliminarCotizacion.php?id_cotizacion=${id}`,
        {
          method: "GET",
        }
      );
  ...
  return (
    <Home
      children1={
        <>
          <Encabezado
            titlle={"Orden de Servicio Masivo"}
            btntittle={"Generar"}
