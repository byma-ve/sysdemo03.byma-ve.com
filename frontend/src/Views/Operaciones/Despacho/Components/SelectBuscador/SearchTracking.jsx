...
export default function SearchTracking({
  ...
  useEffect(() => {
    if (selectedGuiaUnitaria) {
      const selected = trackings.find((t) => t.id_num_guia_registro_carga === selectedGuiaUnitaria);
    ...
    <Combobox value={selectedTracking} onChange={handleSelectTracking}>
      <div className="w-[90%] mt-1 h-5 text-xs bg-gray-100 px-1 rounded-sm border focus:outline-none focus:ring-0 focus:border-blue-500 focus:shadow-md flex truncate">
        <Combobox.Input
          ...
          displayValue={(tracking) => (tracking ? tracking.id_num_guia_registro_carga : "")}
      ...
      >
        {filteredTrackings.length > 0 ? (
          <Combobox.Options className="absolute mt-1 max-h-20 overflow-auto w-[136px] rounded-sm bg-gray-100 py-1 text-base shadow-lg focus:outline-none sm:text-sm">
            {filteredTrackings.map((tracking) => (
              <Combobox.Option
                key={tracking.id_num_guia_registro_carga}
                className={({ active }) =>
                  `relative cursor-default text-xs select-none ps-1 pr-4 ${
                    active ? "bg-gray-300 text-white" : "text-gray-900"
                  } ${
                    selectedTracking && selectedTracking.id_num_guia_registro_carga === tracking.id_num_guia_registro_carga
                      ? "bg-gray-300"
                      : ""
                  }`
                }
