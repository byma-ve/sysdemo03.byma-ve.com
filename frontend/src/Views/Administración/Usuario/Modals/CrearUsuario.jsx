...
function ModalCrearUsuario({ modalNuevo, setModalNuevo,actualizarTabla }) {
  ...
  const handleSubmit = async (e) => {
    ...
    formDataWithImage.append("id_creador_usuario", localStorage.getItem('id_usuario'));
    formDataWithImage.append("dni_usuario", formData.dni_usuario);
    formDataWithImage.append("clave_usuario", formData.clave_usuario);
    formDataWithImage.append("colaborador_usuario", formData.colaborador_usuario);
    ...
    try {
      const response = await fetch('https://sysdemo03.byma-ve.com/BackendApiRest/Administracion/Usuario/guardar_usuario.php', {
        method: 'POST',
      ...
      if (response.ok) {
        ...
        const errorMessage = errorResponse.error || 'Hubo un error al intentar guardar los datos.';
  ...
  return (
    <>
      <div
        className={`side-panel-container ${modalNuevo ? "visible" : "invisible"
          } fixed pointer-events-auto left-0 top-0 right-0 h-full bg-[rgba(0,0,0,0.4)] z-10`}
      >
        <div
          className={`side-panel-cont-container ${modalNuevo ? "translate-x-[0%]" : "translate-x-[100%]"
              ...
              <div className="side-panel  h-full w-auto m-0 ">
                ...
                <div>
                  <div className="section-crm px-6">
                    ...
                    <form className="text-black" key={resetForm} onSubmit={handleSubmit} encType="multipart/form-data">
                        ...
                        <div className="relative z-0 w-full mb-6 group">
                          <select onChange={handleInputChange}
                            ...
                            placeholder=" "
                            required>
                            <option value="" className="bg-gray-200">Elegir Cargo...</option>
                            <option value="auxiliar" className="bg-gray-200">Auxiliar de Distribución</option>
                            <option value="conductor" className="bg-gray-200">Conductor</option>
                            <option value="vendedor" className="bg-gray-200">Vendedor</option>
                            <option value="jefe de operaciones" className="bg-gray-200">Jefe de operaciones</option>
                            <option value="auxiliar logistico" className="bg-gray-200">Auxiliar Logistico</option>
                            <option value="coordinador logistico" className="bg-gray-200">Coordinador Logistico</option>
                            <option value="atencion al cliente" className="bg-gray-200">Atencion al cliente</option>
                            <option value="auxiliar contable" className="bg-gray-200">Auxiliar contable</option>
                            <option value="gerente comercial" className="bg-gray-200">Gerente comercial</option>
                            <option value="gerente general" className="bg-gray-200">Gerente general</option>
                            <option value="supervisor logistico" className="bg-gray-200">Supervisor Logistico</option>
                            <option value="administrador de sistema" className="bg-gray-200">Administrador de Sistema</option>
                            <option value="asistente contable" className="bg-gray-200">Asistente contable</option>
                          </select>
                          <label className="upp peer-focus:font-medium absolute text-sm text-gray-500  duration-300 transform -translate-y-6 scale-75 top-3 -z-10 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 peer-focus:text-blue-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-90 peer-focus:-translate-y-6">
                            Cargo *
                          </label>
