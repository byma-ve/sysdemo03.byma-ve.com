...
function guardarCotizacion($datos)
{
    ...
    if (
        ...
        $stmt = $bd->prepare("SELECT MAX(id_cotizacion) AS max_id FROM cotizaciones WHERE id_cotizacion LIKE 'C1-%'");
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        $max_id_cotizacion = $row['max_id'];
        if ($max_id_cotizacion === NULL) {
            $id_cotizacion = 'C1-000001';
        } else {
            $numeric_part = substr($max_id_cotizacion, 3) + 1;
            $id_cotizacion = 'C1-' . str_pad($numeric_part, 6, '0', STR_PAD_LEFT);
        }
        ...
        WHERE cd.id_cotizacion_cotizacion_destino = 'C1-000000'
        AND c.id = :id_cliente
        AND a.id = :id_area
        ;";
        $stmt = $bd->prepare($sql);
        ...
        if ($cant_destinos != 0) {
            ...
            if (!empty($mostrar_cli['id_vendedor_usuario_cliente'])) {
                ...
                if ($success) {
                    $sql_upt_ = "UPDATE cotizaciones_destinos SET id_cotizacion_cotizacion_destino = :id_cotizacion  WHERE id_cotizacion_cotizacion_destino = 'C1-000000' 
...
function obtenerCalculoTotal($id_cliente, $id_area)
{
    ...
    $stmt = $bd->prepare("
            ...
            WHERE 
                ...
                AND cd.id_cotizacion_cotizacion_destino = 'C1-000000'
        ");
    $stmt->bindParam(':id_cliente', $id_cliente);
    $stmt->bindParam(':id_area', $id_area);
    $stmt->execute();
...
function obtenerListaDestinos($id_cliente, $id_area)
{
    ...
    AND cd.id_area_cotizacion_destino = :id_area AND cd.id_cotizacion_cotizacion_destino = 'C1-000000' ORDER BY cd.id DESC;");
...
function guardarDestino($datos)
{
    ...
    if (
        ...
        if ($result) {
            ...
            $data = array(
                "id_cotizacion_cotizacion_destino" => "C1-000000",
                "id_cliente_cotizacion_destino" => $datos['id_cliente'],
                "id_area_cotizacion_destino" => $datos['id_area'],
                "consignado_cotizacion_destino" => $datos['consignado_cotizacion_destino'],
                "dni_ruc_cotizacion_destino" => $datos['dni_ruc_cotizacion_destino'],
