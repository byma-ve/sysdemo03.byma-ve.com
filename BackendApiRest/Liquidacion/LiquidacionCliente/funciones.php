...
function crearEstructuraCarpetasMensual($servicio, $idCarpetaRaiz, $anio, $mes, $cliente)
{
    ...
    $idCarpetaClientes = crearCarpetaSinoExiste($servicio, $idCarpetaPdfs, "Clientes");

    $idCarpetaCliente = crearCarpetaSinoExiste($servicio, $idCarpetaClientes, $cliente);
...
function guardarImagen($pdf_liquidacion_cliente, $orden_servicio_liquidacion_cliente, $cliente, $num_documento)
{
    ...
    $file->setName($orden_servicio_liquidacion_cliente . ' / ' . $cliente . ' / ' . $num_documento); // Nombre del Archivo
...
function guardarLiquidacion($datos)
{
    ...
    //PUEDE QUE HAYA UN ERROR CUANDO NO SE HAYA PROGRAMADO TODAVIA LA ORDEN DE SERVICIO SALDRA ERROR DE RED

    if ($validacion_db['total'] > 0) {
        if (!empty($datos->pdf_liquidacion_cliente)) {
            $rutaDB = guardarImagen($datos->pdf_liquidacion_cliente, $datos->orden_servicio_liquidacion_cliente, $cliente, $datos->num_documento_liquidacion_cliente);
