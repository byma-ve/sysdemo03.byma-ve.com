...
function guardarRegistroMasivo($data)
{
    ...
    foreach ($data as $registro) {
        ...
        if (!$existe) {
            if (substr($id_orden_servicio_estado_recojo, 0, 2) === "C1") {
            ...
            } elseif (substr($id_orden_servicio_estado_recojo, 0, 2) === "C2") {
            ...
            } elseif (substr($id_orden_servicio_estado_recojo, 0, 3) === "OS1") {
            ...
            } elseif (substr($id_orden_servicio_estado_recojo, 0, 3) === "OS2") {
            ...
            if (substr($id_orden_servicio_estado_recojo, 0, 3) === "OS2") {
            ...
            if ($tarifario == 'Courrier') {
                $stmt = $bd->prepare("SELECT MAX(id_num_guia_registro_carga) AS max_id FROM registros_cargas WHERE id_num_guia_registro_carga LIKE 'MR2%'");
                $stmt->execute();
                $row = $stmt->fetch(PDO::FETCH_ASSOC);
                $max_id_cotizacion = $row['max_id'];

                if ($max_id_cotizacion === NULL) {
                    $num_guia = 'MR200000001';
                } else {
                    $numeric_part = intval(substr($max_id_cotizacion, 2)) + 1;
                    $num_guia = 'MR' . str_pad($numeric_part, 9, '0', STR_PAD_LEFT);
                }
            } else if ($tarifario == 'Aerea') {
                $stmt = $bd->prepare("SELECT MAX(id_num_guia_registro_carga) AS max_id FROM registros_cargas WHERE id_num_guia_registro_carga LIKE 'MR6%'");
                $stmt->execute();
                $row = $stmt->fetch(PDO::FETCH_ASSOC);
                $max_id_cotizacion = $row['max_id'];
                if ($max_id_cotizacion === NULL) {
                    $num_guia = 'MR600000001';
                } else {
                    $numeric_part = intval(substr($max_id_cotizacion, 2)) + 1;
                    $num_guia = 'MR' . str_pad($numeric_part, 9, '0', STR_PAD_LEFT);
                }
            } else if ($tarifario == 'Carga') {
                $stmt = $bd->prepare("SELECT MAX(id_num_guia_registro_carga) AS max_id FROM registros_cargas WHERE id_num_guia_registro_carga LIKE 'MR4%'");
                $stmt->execute();
                $row = $stmt->fetch(PDO::FETCH_ASSOC);
                $max_id_cotizacion = $row['max_id'];
                if ($max_id_cotizacion === NULL) {
                    $num_guia = 'MR400000001';
                } else {
                    $numeric_part = intval(substr($max_id_cotizacion, 2)) + 1;
                    $num_guia = 'MR' . str_pad($numeric_part, 9, '0', STR_PAD_LEFT);
                }
            } else if ($tarifario == 'Valorizada') {
                $stmt = $bd->prepare("SELECT MAX(id_num_guia_registro_carga) AS max_id FROM registros_cargas WHERE id_num_guia_registro_carga LIKE 'MR8%'");
                $stmt->execute();
                $row = $stmt->fetch(PDO::FETCH_ASSOC);
                $max_id_cotizacion = $row['max_id'];
                if ($max_id_cotizacion === NULL) {
                    $num_guia = 'MR800000001';
                } else {
                    $numeric_part = intval(substr($max_id_cotizacion, 2)) + 1;
                    $num_guia = 'MR' . str_pad($numeric_part, 9, '0', STR_PAD_LEFT);
                }
...
function guardarRegistroCarga($data)
{
    ...
    if (substr($id_orden_servicio, 0, 2) === "C1") {
    ...
    } elseif (substr($id_orden_servicio, 0, 2) === "C2") {
    ...
    } elseif (substr($id_orden_servicio, 0, 3) === "OS1") {
    ...
    } elseif (substr($id_orden_servicio, 0, 3) === "OS2") {
