...
function guardarCotizacion($datos)
{
    ...
    if (
        ...
        $stmt = $bd->prepare("SELECT MAX(id_registro_masivo) AS max_id FROM registro_masivos WHERE id_registro_masivo LIKE 'OS2-%'");
        $stmt->execute();
        $row = $stmt->fetch(PDO::FETCH_ASSOC);
        $max_id_cotizacion = $row['max_id'];
        if ($max_id_cotizacion === NULL) {
            $id_cotizacion = 'OS2-000001';
        } else {
            $numeric_part = substr($max_id_cotizacion, 4) + 1;
            $id_cotizacion = 'OS2-' . str_pad($numeric_part, 6, '0', STR_PAD_LEFT);
        }
        ...
        WHERE cd.id_registro_masivo_destino = 'OS2-000000'
        AND c.id = :id_cliente
        AND a.id = :id_area
        ;";
        $stmt = $bd->prepare($sql);
        ...
        if ($cant_destinos != 0) {
            ...
            if (!empty($mostrar_cli['id_vendedor_usuario_cliente'])) {
                ...
                if ($success) {
                    ...
                    $sql_upt_ = "UPDATE registro_masivo_destinos SET id_registro_masivo_destino = :id_cotizacion  WHERE id_registro_masivo_destino = 'OS2-000000' 
...
function obtenerCalculoTotal($id_cliente, $id_area)
{
    ...
    $stmt = $bd->prepare("
            ...
            WHERE 
                ...
                AND cd.id_registro_masivo_destino = 'OS2-000000'
        ");
    $stmt->bindParam(':id_cliente', $id_cliente);
    $stmt->bindParam(':id_area', $id_area);
    $stmt->execute();
...
function obtenerListaDestinos($id_cliente, $id_area)
{
    ...
    AND cd.id_area_registro_masivo_destino = :id_area AND cd.id_registro_masivo_destino = 'OS2-000000' ORDER BY cd.id DESC;");
    ...
    if (is_array($datos) && count($datos) > 0) {
        if (count($datos) > 0) {
            foreach ($datos as $fila => $dato) {
                foreach ([
                    'CONSIGNADO', 'RUC/DNI', 'TELÉFONO', 'TELÉFONO-EX', 'DIRECCIÓN', 'REFERENCIAS', 'TIPO-ENVIO', 'DEPARTAMENTO', 'PROVINCIA', 'DISTRITO', 'TIPO-MOVIMIENTO', 'TIPO-LOGISTICA', 'CONTENIDO-MERC', 'PESO-MERC', 'CANT-MERC', 'LARGO', 'ANCHO', 'ALTO', 'VALOR-MERCANCÍA'
                ] as $campo) {
                ...
                $camposNumericos = ['RUC/DNI', 'TELÉFONO', 'PESO-MERC', 'CANT-MERC', 'LARGO', 'ANCHO', 'ALTO', 'VALOR-MERCANCÍA'];
            ...
            foreach ($datos as $fila => $dato) {
                ...
                $resultadoAdicional = totalAdicionales(floatval($dato['VALOR-MERCANCÍA']), 0, 0, 0, 0, 0);
                ...
                $data = array(
                    "id_registro_masivo_destino" => "OS2-000000",
                    ...
                    "valor_mercancia_registro_masivo_destino" => $dato['VALOR-MERCANCÍA'],
                    ...
                    "documento_1_registro_masivo_destino" => isset($dato['DOC-ADICIONAL(1)']) && !empty($dato['DOC-ADICIONAL(1)']) ? $dato['DOC-ADICIONAL(1)'] : '-',
                    "documento_2_registro_masivo_destino" => isset($dato['DOC-ADICIONAL(2)']) && !empty($dato['DOC-ADICIONAL(2)']) ? $dato['DOC-ADICIONAL(2)'] : '-',
